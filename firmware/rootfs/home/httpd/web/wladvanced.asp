<%SendWebHeadStr(); %>
<title><% multilang("149" "LANG_WLAN_ADVANCED_SETTINGS"); %></title>
...
function validateNum(str)
{
  ...
  alert("<% multilang("2449" "LANG_INVALID_VALUE_IT_SHOULD_BE_IN_DECIMAL_NUMBER_0_9"); %>");
  return false;
   }
  }
  return true;
...
function saveChanges()
{
 ...
 return false;
  ...
  if (document.advanceSetup.fragThreshold.value == "" || num < 256 || num > 2346) {
   alert("<% multilang("2450" "LANG_INVALID_VALUE_OF_FRAGMENT_THRESHOLD_INPUT_VALUE_SHOULD_BE_BETWEEN_256_2346_IN_DECIMAL"); %>");
   document.advanceSetup.fragThreshold.focus();
 return false;
  }
 ...
 return false;
  ...
  if (document.advanceSetup.rtsThreshold.value=="" || num > 2347) {
   alert("<% multilang("2451" "LANG_INVALID_VALUE_OF_RTS_THRESHOLD_INPUT_VALUE_SHOULD_BE_BETWEEN_0_2347_IN_DECIMAL"); %>");
   document.advanceSetup.rtsThreshold.focus();
 return false;
  }
 ...
 return false;
  ...
  if (document.advanceSetup.beaconInterval.value=="" || num < 20 || num > 1024) {
   alert("<% multilang("2452" "LANG_INVALID_VALUE_OF_BEACON_INTERVAL_INPUT_VALUE_SHOULD_BE_BETWEEN_20_1024_IN_DECIMAL"); %>");
   document.advanceSetup.beaconInterval.focus();
 return false;
  }
  return true;
...
function stactrlPreBandChange()
{
 if(document.advanceSetup.elements.namedItem("stactrl_prefer_band")!= null){
  with (document.advanceSetup) {
   if(document.advanceSetup.elements.namedItem("sta_control").value == 1)
    document.advanceSetup.elements.namedItem("stactrl_prefer_band").disabled = false;
   else
    document.advanceSetup.elements.namedItem("stactrl_prefer_band").disabled = true;
  }
 }
}
</SCRIPT>
<blockquote>
<body>
<!-- <h2 class="page_title"><% multilang("149" "LANG_WLAN_ADVANCED_SETTINGS"); %></h2> -->
<form action=/boaform/admin/formAdvanceSetup method=POST name="advanceSetup">
<div class="intro_main ">
 <p class="intro_title"<% multilang("149" "LANG_WLAN_ADVANCED_SETTINGS"); %>></p> <br>
 <p class="intro_content"> <% multilang("150" "LANG_PAGE_DESC_WLAN_ADVANCE_SETTING"); %></p><br>
</div>
<div class="data_common data_common_notitle">
 <table>
  <input type=hidden name="wlanDisabled" value=<% wlanStatus(); %>
      <tr <% checkWrite("wlan_qtn_hidden_function"); %>>
         <th width=40%><% multilang("151" "LANG_FRAGMENT_THRESHOLD"); %>:</th>
         <td><input type="text" name="fragThreshold" size="10" maxlength="4" value=<% getInfo("fragThreshold"); %>>(256-2346)</td>
      </tr>
      <tr>
         <th width=40%><% multilang("152" "LANG_RTS_THRESHOLD"); %>:</th>
         <td><input type="text" name="rtsThreshold" size="10" maxlength="4" value=<% getInfo("rtsThreshold"); %>>(0-2347)</td>
      </tr>
  <tr>
   <th width=40%><% multilang("153" "LANG_BEACON_INTERVAL"); %>:</th>
   <td><input type="text" name="beaconInterval" size="10" maxlength="4" value=<% getInfo("beaconInterval"); %>> (20-1024 <% multilang("941" "LANG_MS"); %>)</td>
  </tr>
  <tr>
   <th width=40%><% multilang("154" "LANG_DATA_RATE"); %>:</th>
...
if(!is_wlan_qtn){
 ...
 rate_name =["<% multilang("155" "LANG_AUTO"); %>","1M","2M","5.5M","11M","6M","9M","12M","18M","24M","36M","48M","54M", "MCS0", "MCS1",
 ...
 idx=0, i=0;
 document.write('<option value="' + (i) + '">' +'<% multilang("155" "LANG_AUTO"); %>'+ '\n');
 idx++;
 defidx=0;
 if ((band & 8) || (band&4)) {
 ...
 document.advanceSetup.txRate.selectedIndex=defidx;
  ...
  <!-- for WiFi test, start --
    <tr>
      <td width="30%"><font size=2><b><% multilang("156" "LANG_TX_OPERATION_RATE"); %>:</b></td>
 ...
 <input type="checkbox" name="operRate48M" value="48M">28M&nbsp;&nbsp;
    ...
    <tr>
      <td width="30%"><font size=2><b><% multilang("157" "LANG_TX_BASIC_RATE"); %>:</b></td>
      <td width="70%"><font size=2>
 ...
 <input type="checkbox" name="basicRate48M" value="48M">28M&nbsp;&nbsp;
    ...
    <tr>
      <td width="30%"><font size=2><b><% multilang("158" "LANG_DTIM_PERIOD"); %>:</b></td>

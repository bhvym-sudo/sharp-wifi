<%SendWebHeadStr(); %>
<title><% multilang("213" "LANG_WI_FI_PROTECTED_SETUP"); %></title>
...
function genPinClicked()
{
 ...
 alert('<% multilang("3589" "LANG_WPS_WARN_MSG_3"); %>');
}
...
function setPinClicked(form)
{
 ...
 if (ret == 1) {
  alert('<% multilang("2497" "LANG_INVALID_ENROLLEE_PIN_LENGTH_THE_DEVICE_PIN_IS_USUALLY_FOUR_OR_EIGHT_DIGITS_LONG"); %>');
  form.peerPin.focus();
  return false;
 }
 else if (ret == 2) {
  alert('<% multilang("2498" "LANG_INVALID_ENROLLEE_PIN_ENROLLEE_PIN_MUST_BE_NUMERIC_DIGITS"); %>');
  form.peerPin.focus();
  return false;
 }
 else if (ret == 3) {
  if ( !confirm('<% multilang("2524" "LANG_CHECKSUM_FAILED_USE_PIN_ANYWAY"); %>') ) {
   form.peerPin.focus();
   return false;
    }
 }
...
function saveChangesWPS(form)
{
 ...
 if (ret == 1) {
  alert('<% multilang("2499" "LANG_INVALID_PIN_LENGTH_THE_DEVICE_PIN_IS_USUALLY_FOUR_OR_EIGHT_DIGITS_LONG"); %>');
  form.localPin.focus();
  return false;
 }
 else if (ret == 2) {
  alert('<% multilang("2500" "LANG_INVALID_PIN_THE_DEVICE_PIN_MUST_BE_NUMERIC_DIGITS"); %>');
  form.localPin.focus();
  return false;
 }
 else if (ret == 3) {
  alert('<% multilang("2501" "LANG_INVALID_PIN_CHECKSUM_ERRO"); %>');
  form.localPin.focus();
  return false;
 }
 ...
 if (wps_enc_status) {
  alert('<% multilang("2502" "LANG_WPS2_0_DOES_NOT_SUPPORT_THIS_ENCRYPTION_MODE"); %>');
  return false;
 }
    return true;
}
...
<body>
<blockquote>
<!-- <h2 class="page_title"><% multilang("213" "LANG_WI_FI_PROTECTED_SETUP"); %></h2> -->
<form action=/boaform/formWsc method=POST name="formWsc">
<div class="intro_main ">
 <p class="intro_title"><% multilang("213" "LANG_WI_FI_PROTECTED_SETUP"); %></p><br>
 <p class="intro_content"><% multilang("214" "LANG_PAGE_DESC_WPS_SETTING"); %></p><br>
</div>
...
<tr>
  <th width=40%><% multilang("215" "LANG_DISABLE_WPS"); %></th>
  <td><input type="checkbox" name="disableWPS" value="ON" <% checkWrite("wscDisable"); %> ONCLICK="checkWPSstate(document.formWsc)"></td>
</tr>
<script>
  if (isClient) {
...
<tr>
  ...
  <td>
   <input type="radio" name="config" value="on" <% checkWrite("wscConfig-1"); %>><% multilang("216" "LANG_CONFIGURED"); %>&nbsp;&nbsp;
   <input type="radio" name="config" value="off" <% checkWrite("wscConfig-0"); %>><% multilang("217" "LANG_UNCONFIGURED"); %>
</td>
</tr>
<script>
  if (isClient) {
...
<script>
  ...
  <th width=40%>WPS <% multilang("670" "LANG_VERSION"); %>:</th>
  <td>
...
<tr <% checkWrite("wlan_qtn_hidden_function"); %>>
  <th width=40%><% multilang("218" "LANG_AUTO_LOCK_DOWN_STATE"); %>:</th>
  <td>
<script>
  if (autolockdown_stat) {
 document.write("<% multilang("219" "LANG_LOCKED"); %>");
  }else{
 document.write("<% multilang("220" "LANG_UNLOCKED"); %>");
  }
</script>
   <input class="inner_btn" type="submit" value="<% multilang("221" "LANG_UNLOCK"); %>" name="unlockautolockdown" onClick="return unlockdownClicked()">
 </td>
</tr>
<tr>
  <th width=40%><% multilang("222" "LANG_SELF_PIN_NUMBER"); %>:</th>
  <td><input type="text" name="localPin" size="12" maxlength="10" onkeypress="return noenter(event)" value=<% getInfo("wscLoocalPin"); %>>
   &nbsp;&nbsp;<input type="button" value="<% multilang("223" "LANG_REGENERATE_PIN"); %>" name="genPIN" onClick="return genPinClicked()"></td>
</tr>
<script>
  if (!isClient) {
 document.write("</table>\n");
...
<tr>
  <th width=40%>PIN <% multilang("224" "LANG_CONFIGURATION"); %>:</th>
  <td>
   <input type="submit" value="<% multilang("225" "LANG_START_PIN"); %>" name="triggerPIN" onClick="return triggerPINClicked()"></td>
 </td>
</tr>
<script>
  if (!isClient) {
...
<tr>
  <th width=40%><% multilang("226" "LANG_PUSH_BUTTON"); %> <% multilang("224" "LANG_CONFIGURATION"); %>:</th>
  <td>
   <input type="submit" value="<% multilang("227" "LANG_START_PBC"); %>" name="triggerPBC" onClick="return triggerPBCClicked()"></td>
</tr>
</table>
</div>
<br>
<div class="btn_ctl">
 ...
 <input class="link_bg" type="submit" value="<% multilang("138" "LANG_APPLY_CHANGES"); %>" name="save" onClick="return saveChangesWPS(document.formWsc)">&nbsp;&nbsp;
...
<br><br>
<div class="intro_main ">
 <p class="intro_title"><% multilang("228" "LANG_CURRENT_KEY_INFO"); %></p>
</div>
<div class="data_common data_common_notitle data_vertical">
<table>
<tr>
   <th><% multilang("188" "LANG_AUTHENTICATION"); %></th>
   <th><% multilang("187" "LANG_ENCRYPTION"); %></th>
   <th><% multilang("229" "LANG_KEY"); %></th>
</tr>
<tr>
   <td width="30%"><% checkWrite("wps_auth"); %></td>
   <td width="20%"><% checkWrite("wps_enc"); %></td>
   <td width="50%"><% getInfo("wps_key"); %></td>
</tr>
</table>
</div>
<script>
  if (isClient || !isConfig) {
 document.write("</span>\n");
 document.write("<div class="data_common data_common_notitle"><table>\n");
  }
  if (isClient)
 document.write("<span id = \"hide_div\" class = \"off\">\n");
</script>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
     <th width=40%><% multilang("230" "LANG_CLIENT_PIN_NUMBER"); %>:</th>
     <td><input type="text" name="peerPin" size="12" maxlength="10" onkeypress="return noenter(event)" value="">
       &nbsp;&nbsp;<input type="submit" value="<% multilang("225" "LANG_START_PIN"); %>" name="setPIN" onClick="return setPinClicked(document.formWsc)">

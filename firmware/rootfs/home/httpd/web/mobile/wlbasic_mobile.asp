<%SendWebHeadStr("mobile_normal_3"); %>
<title></title>
<style>
.on {display:on}
.off {display:none}
...
function saveChanges()
{
 ...
 if (document.wlanSetup.ssid.value=="") {
  alert("<% multilang("2453" "LANG_SSID_CANNOT_BE_EMPTY"); %>");
...
*/
 ...
 if(document.wlanSetup.wl_limitstanum.value == 1){
  if(!isNumber(document.wlanSetup.wl_stanum.value)){
   alert("<% multilang("2454" "LANG_INVALID_LIMIT_ASSOCIATED_CLIENT_NUMBER"); %>");
...
function disableWLAN()
{
 ...
 disableTextField(document.wlanSetup.wlHide);
 disableTextField(document.wlanSetup.wl_access);
 disableTextField(document.wlanSetup.wl_wmm_func);
 disableTextField(document.wlanSetup.shortGI0);
 disableTextField(document.wlanSetup.powerincrease);
 disableTextField(document.wlanSetup.powersaving);
 ...
 disableButton(document.wlanSetup.PasswordSetting);
}
function enableWLAN()
{
 ...
 enableTextField(document.wlanSetup.wlHide);
 enableTextField(document.wlanSetup.wl_access);
 enableTextField(document.wlanSetup.wl_wmm_func);
 enableTextField(document.wlanSetup.shortGI0);
 enableTextField(document.wlanSetup.powerincrease);
 enableTextField(document.wlanSetup.powersaving);
 ...
 enableButton(document.wlanSetup.PasswordSetting);
...
function showSetPassword(form,url)
{
 if (!form.wlanDisabled.checked)
  document.location.href = url;
}
...
//	var band_idx = 0;
 ...
 if(band_value < 7) {
  ...
  if(document.getElementById)
   document.getElementById('optionfor11n').style.display = '';
  else if(document.layers == false)
   document.all.optionfor11n.style.display = '';
 }
/*  
	while(band_idx < document.wlanSetup.band.length) {
...
function showChannel5G(startChanIdx, bound_40)
{
 ...
 if (startChanIdx == 0) {
  if (dfs_enable == 1)
  {
   /*
			if(withoutdfs == 0){
				document.wlanSetup.chan.options[0] = new Option("Auto(Without DFS)", 0, false, false);
			}
			else{
				document.wlanSetup.chan.options[0] = new Option("Auto(DFS)", 0, false, false);
			}
			*/
   /* Modified by jzh for bug#0003853 */
   document.wlanSetup.chan.options[0] = new Option("Auto(DFS)", 0, false, false);
   document.wlanSetup.chan.options[1] = new Option("Auto(Without DFS)", 20, false, false);
   startChanIdx ++;
   /* End of modified for bug#0003853 */
  }
  else
  ...
  /* Modified by jzh for bug#0003853 */
  else if (1 == defaultChan)
  {
   document.wlanSetup.chan.selectedIndex = 1;
   defChanIdx = 1;
  }
  /* End of modified for bug#0003853 */
  startChanIdx ++;
...
function showChannel2G(bound_40, band)
{
 ...
 document.wlanSetup.chan.length=0;
 idx=0;
 document.wlanSetup.chan.options[idx] = new Option("<% multilang("155" "LANG_AUTO"); %>", 0, false, false);
 if (0 == wlan_channel) {
  document.wlanSetup.chan.selectedIndex = 0;
 }
 idx++;
...
function updateWlanAcc()
{
 if(document.wlanSetup.wlanAcc.checked == true) {
  document.wlanSetup.wlanAcc.value="ON";
 }
 else {
  document.wlanSetup.wlanAcc.value="OFF";
 }
}
function updateWlanAccess()
{
 if(document.wlanSetup.wl_access.checked == true) {
  document.wlanSetup.wl_access.value="ON";
 }
 else {
  document.wlanSetup.wl_access.value="OFF";
 }
}
function updateWlanBssid()
{
 if(document.wlanSetup.wlHide.checked == true) {
  document.wlanSetup.wlHide.value="ON";
 }
 else {
  document.wlanSetup.wlHide.value="OFF";
 }
}
function updateWlanwmm()
{
 if(document.wlanSetup.wl_wmm_func.checked == true) {
  document.wlanSetup.wl_wmm_func.value="ON";
 }
 else {
  document.wlanSetup.wl_wmm_func.value="OFF";
 }
}
function updateWlanSGI()
{
 if(document.wlanSetup.shortGI0.checked == true) {
  document.wlanSetup.shortGI0.value="ON";
 }
 else {
  document.wlanSetup.shortGI0.value="OFF";
 }
}
function updateWlanpowerincrease()
{
 if(document.wlanSetup.powerincrease.checked == true) {
  document.wlanSetup.powerincrease.value="ON";
 }
 else {
  document.wlanSetup.powerincrease.value="OFF";
 }
}
function updateWlanpowersaving()
{
 if(document.wlanSetup.powersaving.checked == true) {
  document.wlanSetup.powersaving.value="ON";
 }
 else {
  document.wlanSetup.powersaving.value="OFF";
 }
}
</SCRIPT>
</head>
<body onLoad="updateState(document.wlanSetup);">
...
<div class="page">
 <div class="simplebox">
   <table class="tabletitle">
    <tr>
     <td><% multilang("125" "LANG_WLAN_BASIC_SETTINGS"); %></td>
    </tr>
   </table>
   <table class="tabledata">
    <tr>
     <td width=40%><% multilang("127" "LANG_DISABLE_WLAN_INTERFACE"); %></td>
     <td><input type="checkbox" name="wlanDisabled" value=<% wlanStatus(); %> ONCLICK=updateState(document.wlanSetup)></td>
    </tr>
    <tr>
     <td width=40%><% multilang("128" "LANG_BAND"); %>:</td>
     <td><select size=1 name=band onChange="updateChan(); Set_SSIDbyBand(document.wlanSetup);"><% checkWrite("wlband"); %></select><SCRIPT>updateBand_selectedIndex(defaultBand);</SCRIPT></td>
    </tr>
    <tr>
     <td width=40%><% multilang("129" "LANG_MODE"); %>:</td>
     <td><select size="1" name="mode" onChange="updateMode()"><% checkWrite("wlmode"); %></select>
      &nbsp;&nbsp;&nbsp;&nbsp;
      <input class="submit-button" type="button" value="<% multilang("141" "LANG_MULTIPLE_AP"); %>" name="multipleAP" onClick="showMultipleAP(document.wlanSetup, '/mobile/wlmultipleap_mobile.asp')">
     </td>
    </tr>
    <tr>
     <td width=40%><% multilang("130" "LANG_SSID"); %>:</td>
     <td><input type=text name=ssid size="25" maxlength="32" value="<% getInfo("ssid"); %>"></td>
    </tr>
    <tr>
     <td width=40%><% multilang("3588" "LANG_CANCEL_BROADCAST"); %>:</td>
      <td><input type='checkbox' name='wlHide' ONCLICK="updateWlanBssid()" value=<% wlanssidhide(); %> ></td>
    </tr>
    <tr>
     <td width=40%><% multilang("3621" "LANG_BLOCK_RELAY"); %></td>
      <td><input type='checkbox' name='wl_access' ONCLICK="updateWlanAccess()" value=<% wlanaccess(); %>></td>
    </tr>
    <tr>
     <td width=40%>WMM:</td>
      <td><input type='checkbox' name='wl_wmm_func' ONCLICK="updateWlanwmm()" value=<% wlanwmmfunc(); %>></td>
    </tr>
    <tr>
     <td width=40%>SGI:</td>
      <td><input type='checkbox' name='shortGI0' ONCLICK="updateWlanSGI()" value=<% wlansgifunc(); %>></td>
    </tr>
    <tr>
     <td width=40%><% multilang("3623" "LANG_POWER_ENHANCING"); %>:</td>
      <td><input type='checkbox' name='powerincrease' ONCLICK="updateWlanpowerincrease()" value=<% wlanpowerincreasefunc(); %>></td>
    </tr>
    <tr>
     <td width=40%><% multilang("147" "LANG_POWER_SAVING"); %>:</td>
      <td><input type='checkbox' name='powersaving' ONCLICK="updateWlanpowersaving()" value=<% wlanpowersavingfunc(); %>></td>
    </tr>
              </table>
              <table class="tabledata" id="optionfor11n" style="display:none">
               <tr>
     <td width=40%><% multilang("131" "LANG_CHANNEL_WIDTH"); %>:</td>
     <td><select size="1" name="chanwid" onChange="updateChan1()"><% checkWrite("wlchanwid"); %></select></td>
    </tr>
    <tr <% checkWrite("wlan_qtn_hidden_function"); %> >
     <td width=40%><% multilang("132" "LANG_CONTROL_SIDEBAND"); %>:</td>
     <td><select size="1" name="ctlband" onChange="updateChan_channebound()"><% checkWrite("wlctlband"); %></select></td>
    </tr>
              </table>
              <table class="tabledata">
    <tr>
     <td width=40%><% multilang("133" "LANG_CHANNEL_NUMBER"); %>:</td>
     <td><select size="1" name="chan" onChange="updateChan_selectedIndex()"> </select></td>
      <SCRIPT>
       <% checkWrite("wl_chno"); %>
    ...
    <tr <% checkWrite("wlan_qtn_hidden_function"); %>>
     <td width=40%><% multilang("139" "LANG_RADIO_POWER"); %>:</td>
     <td><select size="1" name="txpower"><% checkWrite("txpower"); %></select></td>
    </tr>
    <tr id="wl_limit_stanum" style="display:none">
     <td width=40%><% multilang("135" "LANG_STA_NUMBER"); %>:</td>
     <td><select name=wl_limitstanum onChange="updateWlstanum()">
       <option value="0"><% multilang("165" "LANG_DISABLED"); %></option>
       <option value="1"><% multilang("164" "LANG_ENABLED"); %></option>
        </select>
            <input size="3" maxlength="2" name="wl_stanum">
           </td>
      </tr>
    <tr>
     <td width=40%><% multilang("136" "LANG_ASSOCIATED_CLIENTS"); %>:</td>
     <td><input class="submit-button" type="button" value="<% multilang("137" "LANG_SHOW_ACTIVE_WLAN_CLIENTS"); %>" name="showMac" onClick="showMacClick('/boaform/admin/formWirelessTbl?submit-url=/mobile/wlstatbl_mobile.asp&wlan_idx=<% checkWrite("wlan_idx"); %>')"></td>
    </tr>
    <tr id="wifi_regdomain_demo">
     <td><% multilang("140" "LANG_REGDOMAIN"); %>:</td>
     <td><select size="1" name="regdomain_demo"><% checkWrite("regdomain_list"); %></select></td>
    </tr>
              </table>
    ...
    function show()
    {
     ...
     var showdiv=document.getElementById("wifi_regdomain_demo");
     if(ifcheck.checked)
     {
     ...
     else
     {
      showdiv.style.display="none";
              ...
              <br><br>
        <input type="hidden" value="/mobile/wlbasic_mobile.asp" name="submit-url">
        <input class="submit-button" type="submit" value="<% multilang("138" "LANG_APPLY_CHANGES"); %>" name="save" onClick="return saveChanges()">&nbsp;&nbsp;&nbsp;&nbsp;
        <input class="submit-button" type="button" value="<% multilang("65" "LANG_PASSWORD"); %> <% multilang("356" "LANG_SETTINGS"); %>" name="PasswordSetting" onClick="showSetPassword(document.wlanSetup, '/mobile/wlwpa_mobile.asp')">

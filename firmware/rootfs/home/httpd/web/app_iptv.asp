...
{
 var str=new Array();
 str[0]=new Array(101,"<% multilang("3690" "LANG_INVALID_VLAN_RANGE"); %>"+"\""+ alertVlanStr + "\"");
 str[1]=new Array(102,"<% multilang("3690" "LANG_INVALID_VLAN_RANGE"); %>"+"\""+ alertVlanStr + "\"");
 str[2]=new Array(103,"<% multilang("3690" "LANG_INVALID_VLAN_RANGE"); %>"+"\""+ alertVlanStr + "\"");
...
function getMsgFormArray(str,arg){
 ...
 for(var i=0;i<str.length;i++){
  if(typeof(str[i])=="undefined"){
   alert("<% multilang("3745" "LANG_ARRAY_INIT_ERROR_CHECK_CODE"); %>"+"i = "+i);
   return null;
  }if(errid==str[i][0]){
   if(min ==max&&min ==0){
    if(param==-1){
 ...
 <form id="mcast_vmap" action=/boaform/admin/formMcastVlanMapping method=POST name=mcast_vmap>
  <div class="data_common data_common_notitle">
   <table id="mcast_table">
    <tr>
     <th id="Fnt_mcast_vlanA" width=40%><% multilang("3518" "LANG_MULTICAST_VLAN_TITLE"); %></th>
  ...
  <br><br>
  <div class="btn_ctl">
   <input type="submit" class="link_bg" value="<% multilang("312" "LANG_MODIFY"); %>" id="modify" onclick="return ModifyApply();" />
   <input name="back" type="button" id="back" value="<% multilang("651" "LANG_CANCEL");%>" class="link_bg" onclick="back2add()" />
  </div>
  <div class="data_common data_common_notitle data_vertical">
   <table id="lstrc">
    <tr align="center" nowrap>
     <th><% multilang("68" "LANG_INTERFACE"); %></th>
     <th><% multilang("3519" "LANG_MULTICAST_VLAN"); %></th>
     <th><% multilang("312" "LANG_MODIFY"); %></th>
    </tr>
   </table>
  </div>

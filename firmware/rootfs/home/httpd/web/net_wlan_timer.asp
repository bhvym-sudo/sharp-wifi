...
function convertSSID(SSIDMask)
{
 var ssid_name = ["1", "2", "3", "4", "5", "6", "7", "8"];
 var value="";
 for(i=0;i<8;i++){
  if(SSIDMask & (1<<i))
   value+=ssid_name[i];
...
function addline(index, enable, startTime, endTime, controlCycle, SSIDMask)
{
 var newline;
 ...
 newline.insertCell(-1).innerHTML = convertSSID(SSIDMask);
...
function showTable()
{
 ...
 if (num!=0) {
  for (var i=0; i<num; i++)
   addline(i, _enable[i], _startTime[i], _endTime[i], _controlCycle[i], _SSIDMask[i]);
 }
 else {
 }
...
function Modify(i)
{
 ...
 getObj("ssid_mask").value = 0;
 for (var j=0; j<8; j++) {
  document.forms[0].chkpt[j].checked = (_SSIDMask[i] & (0x1 << j));
 }
...
function back4add()
{
 ...
 getObj("ssid_mask").value = 0;
 for (var j=0; j<8; j++) {
  document.forms[0].chkpt[j].checked = false;
 }
 jslEnDisplay("Btn_Add");
 jslDiDisplay("Btn_Edit");
 jslDiDisplay("back");
...
*/
 for (var j=0; j<8; j++) {
  if(document.forms[0].chkpt[j].checked == true)
   getObj("ssid_mask").value |= (1<<j);
 }
 if(getObj("ssid_mask").value==0){
  alert("请选择SSID 不能为空!");
  return false;
 }
 return true;

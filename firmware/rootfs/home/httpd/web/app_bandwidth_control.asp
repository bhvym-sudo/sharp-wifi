...
function on_init()
{
 ...
 for(var i = 0; i < rules.length; i++)
 {
  ...
  if(rules[i].usBandwidth == 0)
   cell.innerHTML = "";
  else
   cell.innerHTML = rules[i].usBandwidth;
  cell = row.insertCell(2);
  if(rules[i].dsBandwidth == 0)
   cell.innerHTML = "";
  else
...
{
 var str=new Array();
 str[0]=new Array(101,"上下行带宽不能同时为空，请重新输入。");
...
function modifyClick()
{
 ...
 if(rules[act_idx].usBandwidth == 0)
  document.getElementById("modify_us").value="";
 else
  document.getElementById("modify_us").value=rules[act_idx].usBandwidth;
 if(rules[act_idx].dsBandwidth == 0)
  document.getElementById("modify_ds").value="";
 else
...
function ModifyApply()
{
 ...
 {
  var usBand,dsBand;
  usBand = document.getElementById("modify_us").value;
  dsBand = document.getElementById("modify_ds").value;
  if( (usBand == "") && (dsBand == "") )
  {
   var msg = getmsg(101);
   alert(msg);
   getObj("modify_us").focus();
   return false;
  }
...
function back2add()
{
 ...
 setValue("usBandwidth", 0);
 setValue("dsBandwidth", 0);
 setValue("idx",-1);
 getObj("form").submit();
}
  ...
  <DIV align="left" style="padding-left:20px; padding-top:5px">
   <form id="form" action=/boaform/admin/formBandWidth method=POST name="form">
    ...
    <div align="left" style="padding-left:20px;"><br>
     <table id="rulelst" class="flat" border="1" cellpadding="2" cellspacing="0">
      <tr align="center" class="hd">
       ...
       <td width="120px">上行带宽限制(kbps)</td>
       <td width="120px">下行带宽限制(kbps)</td>
       <td>选择</td>
      </tr>
     </table>
    </div>
    ...
    <div id="input_div">
     <hr align="left" class="sep" size="1" width="90%">
     <tr>
      <td id="Mod_us" class="td1"><b>上行(留白表示不限制):&nbsp;</b></td>
      <td class="td2"><input name="modify_us" id="modify_us" type="text" class="inputId" value="" size="15" />&nbsp;kbps</td>
      <td id="Mod_ds" class="td1"><b>&nbsp;&nbsp;&nbsp;下行(留白表示不限制):&nbsp;</b></td>
      <td class="td2"><input name="modify_ds" id="modify_ds" type="text" class="inputId" value="" size="15" />&nbsp;kbps</td>
     </tr>
     <br><br>
     <tr>
     <td class="td1"></td>

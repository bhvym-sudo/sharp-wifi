...
function on_submit()
{
 with ( document.forms[0] )
 {
  ...
  {
   dipStart.focus();
   alert("<% multilang("3418" "LANG_DESTINATION_IP_START_ADDRESS"); %>\"" + sipStart.value + "\"<% multilang("3053" "LANG_IS_INVALID"); %>,<% multilang(TRY_AGAIN); %>!");
   return;
  }
  if(dipEnd.value.length != 0 && sji_checkvip(dipEnd.value) == false)
  {
   dipEnd.focus();
   alert("<% multilang("3419" "LANG_DESTINATION_IP_END_ADDRESS"); %>\"" + dipEnd.value + "\"<% multilang("3053" "LANG_IS_INVALID"); %>,<% multilang(TRY_AGAIN); %>!");
   return;
  }
  ...
  {
   dmask.focus();
   alert("<% multilang("3423" "LANG_DESTINATION_SUBNET_MASK"); %>\"" + dmask.value + "\"<% multilang("3053" "LANG_IS_INVALID"); %>,<% multilang(TRY_AGAIN); %>!");
   return;
  }
  ...
  {
   dportStart.focus();
   alert("<% multilang("3424" "LANG_DESTINATION_START_PORT"); %>\"" + dportStart.value + "\"<% multilang("3053" "LANG_IS_INVALID"); %>,<% multilang(TRY_AGAIN); %>!");
   return;
  }
  if(dportEnd.value.length != 0 && sji_checkdigitrange(dportEnd.value, 1, 65535) == false)
  {
   dportEnd.focus();
   alert("<% multilang("3425" "LANG_DESTINATION_END_PORT"); %>\"" + dportStart.value + "\"<% multilang("3053" "LANG_IS_INVALID"); %>,<% multilang(TRY_AGAIN); %>!");
   return;
  }
...
<body topmargin="0" leftmargin="0" marginwidth="0" marginheight="0" alink="#000000" link="#000000" vlink="#000000" onLoad="on_init();">
 ...
 <div class="intro_main ">
  <p class="intro_title"><% multilang("3393" "LANG_ADD_IP_FILTER_INCOMING"); %></p>
  <p class="intro_content"><% multilang("3395" "LANG_IP_FILTER_WHTADD_1"); %></p>
 ...
 <div class="data_common data_common_notitle">
  <table>
   <tr>
    <th width=30%><% multilang("3399" "LANG_FILTER_NAME"); %>:</th>
 ...
 <div class="data_common data_common_notitle">
  <table>
    ...
    <!--
    <br><br>
    <b>WAN interface (configure and only enable firewall in routing mode)</b><br>
    Select at least one or more WAN interfaces shown below to apply this rule.<br>
    <input type="checkbox" name="allport" onClick="on_sel();"><% multilang("841" "LANG_SELECT_ALL"); %><br>
    <table id="lstrc" border="0" cellpadding="0" cellspacing="0"></table>
    -->
 <div class="btn_ctl">
   <INPUT type="button" class="button" value="<% multilang("138" "LANG_APPLY_CHANGES"); %>" onClick="on_submit();">
